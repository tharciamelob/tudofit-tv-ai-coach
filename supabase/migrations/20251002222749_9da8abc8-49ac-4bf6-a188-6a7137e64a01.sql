-- ============================================
-- MIGRATION: Séries Diversificadas e Coerentes
-- ============================================

-- 1. Adicionar campos de parâmetros em series_exercises
ALTER TABLE series_exercises 
ADD COLUMN IF NOT EXISTS sets INTEGER DEFAULT 3,
ADD COLUMN IF NOT EXISTS reps INTEGER DEFAULT 12,
ADD COLUMN IF NOT EXISTS work_seconds INTEGER,
ADD COLUMN IF NOT EXISTS rest_seconds INTEGER;

-- 2. Função auxiliar: calcular similaridade Jaccard entre dois arrays
...
-- 3. Função: obter exercícios compatíveis com tags
CREATE OR REPLACE FUNCTION get_compatible_exercises(
  required_tags TEXT[],
  category_slug_param TEXT,
  limit_count INT DEFAULT 50
)
RETURNS TABLE(exercise_id UUID, exercise_slug TEXT, tag_match_score INT) AS $$
BEGIN
  RETURN QUERY
  SELECT 
    e.id,
...
-- 4. Função principal: garantir séries variadas
...
-- 5. Função verificadora/auditora
...
-- 6. Executar para todas as categorias (exceto yoga)
DO $$
DECLARE
  v_cat TEXT;
...
BEGIN
  ...
  LOOP
    PERFORM ensure_varied_series(v_cat, 16, 5, false);
  END LOOP;
END $$;